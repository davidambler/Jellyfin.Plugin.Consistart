...
public class SeasonPosterProviderTests
{
    private readonly IConfigurationProvider _configuration;
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly ILogger<SeasonPosterProvider> _logger;
    private readonly IArtworkCandidateService _candidateService;
    private readonly SeasonPosterProvider _provider;

    public SeasonPosterProviderTests()
    {
        _configuration = Substitute.For<IConfigurationProvider>();
        _configuration.BaseUrl.Returns("http://localhost:8096");
        _httpClientFactory = Substitute.For<IHttpClientFactory>();
        _logger = NullLogger<SeasonPosterProvider>.Instance;
        _candidateService = Substitute.For<IArtworkCandidateService>();

        _provider = new SeasonPosterProvider(
            _configuration,
            _httpClientFactory,
            _logger,
            _candidateService
    ...
    [Fact]
    public void Supports_with_season_returns_true()
    {
        var item = new Season();
    ...
    [Fact]
    public void Supports_with_non_season_returns_false()
    {
        var item = Substitute.For<BaseItem>();
    ...
    public async Task GetImages_calls_candidate_service_with_primary_image_type()
    {
        ...
        var candidates = new[]
        {
            new ArtworkCandidateDto("id1", "/render/season1", 500, 750, "en"),
            new ArtworkCandidateDto("id2", "/render/season2", 500, 750, "fr"),
        };
        _candidateService
    ...
    {
        var item = new Season();
        var candidate = new ArtworkCandidateDto("id1", "/render/season.png", 500, 750, "en");
        _candidateService
            .GetCandidatesAsync(item, ImageType.Primary, Arg.Any<CancellationToken>())
        ...
        Assert.Equal("http://localhost:8096/render/season.png", imageInfo.Url);
